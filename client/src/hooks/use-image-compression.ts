    ...
    async (fileId: string) => {
      ...
      try {
        ...
        setStats((prev) => {
          ...
          const newSaved =
            prev.totalSaved + (file.originalSize - compressedBlob.size);
          const avgReduction =
            files.reduce(
              (acc, f) => {
                if (f.compressedSize) {
                  return (
                    acc + calculateSavings(f.originalSize, f.compressedSize)
                  );
                }
                return acc;
              },
              calculateSavings(file.originalSize, compressedBlob.size),
            ) / newProcessed;
    ...
    setFiles((prev) => {
      ...
      if (fileToRemove) {
        ...
        setStats((prevStats) => ({
          ...
          processedFiles:
            fileToRemove.status === "complete"
              ? prevStats.processedFiles - 1
